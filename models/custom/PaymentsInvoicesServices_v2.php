...
class PaymentsInvoicesServices_v2 extends \RightNow\Models\Base
{
    ...
    public function initTransaction($invoice_number, $amount)
    {
        try
        {
            ...
            if ($jsonToken !== false)
            {
              ...
              $url   = "https://api.dimacofi.cl/wp/initTransaction_v2";
              $CI    =&get_instance();
              $CI->load->model('custom/ConnectUrl');
 
              $jsonData                                                    = new \stdClass();
              $jsonData->initTransaction                                   = new \stdClass();
              $jsonData->initTransaction->sessionId                        =  time();
              $jsonData->initTransaction->wSTransactionType                = "TR_NORMAL_WS";
              $jsonData->initTransaction->commerceId                       = "";
              $jsonData->initTransaction->returnURL                        = "https://soportedimacoficl.custhelp.com/cc/Transactions_v2/validTransaction"; //TODO CAMBIAR POR URL DINAMICA
              $jsonData->initTransaction->amount       = round($amount);
              //$jsonData->initTransaction->commerceCode = "597055555532"; // TEST
              $jsonData->initTransaction->commerceCode = "597032782698";  //PROD 597032782698
              $jsonData->initTransaction->buyOrder     = $invoice_number;
    ...
    */
    public function getToken()
    {
      try
      {
          ...
          //$consumerKey    = "yh8wgLIb4RLIHwQ868CIifi2EYca"; // Produccion

          //$consumerSecret = "awZfmg7Q3mgeKzCn6YRMQh_LRBIa";
          $consumerSecret = "uP1Q_Coeio8w_nytC_MuTBfENhga";
          //$consumerSecret = "bfaZkjfdIWoEtiXoDbo4E_EPpAka"; // Produccion
...
/**
    ...
    */
    public function getTransactionResult($token_ws)
    {
      try
      {
          $url = "https://api.dimacofi.cl/wp/getTransactionResult_v2";
          $CI =& get_instance();
          $CI->load->model('custom/ConnectUrl');
          ...
          if ($jsonToken !== false)
          {
            ...
            $jsonDataEncoded = json_encode($jsonData);
            //echo " getTransactionResult ".$jsonDataEncoded ."<br>";
            $service = $CI->ConnectUrl->requestCURLJsonRaw($url, $jsonDataEncoded, $token);
  ...
  /*
    ...
    */
    public function createReceipt($data){
        ...
        if ($jsonToken !== false)
        {
          ...
          $service2 = $CI->ConnectUrl->requestCURLJsonRaw("https://api.dimacofi.cl/wp/createReceipt_v2", $data, $token);
    ...
    */
    public function acknowledgeTransaction($token_ws)
    {
      try
      {
          $url = "https://api.dimacofi.cl/wp/acknowledgeTransaction_v2";
          $CI =& get_instance();
          $CI->load->model('custom/ConnectUrl');
          ...
          if ($jsonToken !== false)
          {
            ...
            $token = $a_jsonToken["access_token"];
             ...
             //echo "ack token wp ".$token_ws."<br>";
             //echo "ack token dimacofi ".$token."<br>";
            ...
            $jsonDataEncoded = json_encode($jsonData);
            //echo " acknowledgeTransaction send ".$jsonDataEncoded ."<br>";
            //exit();
