RightNow.namespace('Custom.Widgets.input.TextInputExtended');
Custom.Widgets.input.TextInputExtended = RightNow.Field.extend({
    overrides: {
    ...
    onValidate: function(type, args) {
        ...
        if(this.data.attrs.display_type === 'rut') {
            errors = this.rutValidation(errors);
        }

        if(this.data.attrs.display_type === 'email') {
            errors = this.emailValidation(errors);
        }

        if(errors.length  || !this.validate(errors) || (this.data.attrs.require_validation && !this._validateVerifyField(errors)) || !this._compareInputToMask(true)) {
    ...
    rutValidation: function(errors) {
        if (this.input.get('value').length && this.data.attrs.display_type === 'rut') {
          if (!Integer.validateRut(Integer.formatRut(this.input.get('value'),2))) {
            errors.push('El valor ingresado no es valido.');
    ...
    emailValidation: function(errors) {
    ...
    _blurValidate: function(event, validateVerifyField) {
        ...
        if (validateVerifyField) {
            ...
            if (valid) {
                ...
                } else if(this.data.attrs.display_type === 'rut') {
                    if (this.rutValidation(errors).length){
                        this.toggleErrorIndicator(true);
                        ...
                        warnDialog = RightNow.UI.Dialog.messageDialog('RUT no válido', {icon: "WARN", exitCallback: {fn: handleOK, scope: this}});
            ...
            if(this.data.attrs.display_type === 'email') {
                if (this.emailValidation(errors).length){
                    this.toggleErrorIndicator(true);
                    //create action dialog with link to acct assistance page
                    var warnDialog,
                        handleOK = function() {
                    ...
                    warnDialog = RightNow.UI.Dialog.messageDialog('Correo Electrónico no válido', {icon: "WARN", exitCallback: {fn: handleOK, scope: this}});
                    this._dialogShowing = true;
                    warnDialog.show();
                }
            }
