RightNow.namespace('Custom.Widgets.input.ProductCategoryInputExtended');
Custom.Widgets.input.ProductCategoryInputExtended = RightNow.Field.extend({
    overrides: {
        constructor: function() {
            ...
            // Exclude items
            this.data.js.hierData[0] = this.exludeItems(this.data.js.hierData[0]);
            if(this.data.js.hierDataNone) this.data.js.hierDataNone[0] = this.exludeItems(this.data.js.hierDataNone[0]);
            if(this.data.js.link_map) this.data.js.link_map[0] = this.exludeItems(this.data.js.link_map[0]);
    ...
    exludeItems: function (ItemsList) {
      console.log('exludeItems: ', ItemsList);
        var Excludes = this.data.attrs.exclude.split(',');
        for (var i=0; i<ItemsList.length; i++) {
            if (Excludes.indexOf(ItemsList[i].id.toString()) !== -1) {
    ...
    getSubLevelResponse: function(type, args) {
        // Exclude items
        args[0].data.hier_data = this.exludeItems(args[0].data.hier_data);
        ...
        if ((evtObj.w_id && evtObj.w_id === this.instanceID) || (this.data.js.linkingOn && evtObj.data.data_type === "Category" && this.data.js.data_type === evtObj.data.data_type)) {
            ...
            if (evtObj.data.reset_linked_category && evtObj.data.data_type === "Category") {
