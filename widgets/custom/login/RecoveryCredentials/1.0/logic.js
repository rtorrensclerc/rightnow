RightNow.namespace('Custom.Widgets.login.RecoveryCredentials');

Custom.Widgets.login.RecoveryCredentials = RightNow.Widgets.extend({
    constructor: function() {
        ...
        RightNow.Event.subscribe('evt_formButtonSubmitRequest', this.formButtonSubmitRequest, this);
        // RightNow.Event.subscribe('evt_formButtonSubmitResponse', this.formButtonSubmitResponse, this);
    },

    // formButtonSubmitResponse: function(evt, obj) {
    //     var value = this.Y.Lang.trim(this.inputField.get('value'));
    //     RightNow.Url.navigate('/app/utils/account_assistance_code/email/' + value);
    // },
    
    formButtonSubmitRequest: function(evt, obj) {
        var value = this.Y.Lang.trim(this.inputField.get('value'));

          obj[0].data.login = value;

        this.Y.one('.rn_TextArea').set('value', JSON.stringify(obj[0]));
    ...
    _onSubmit: function(event, arg) {
        ...
        if(this.credentialType === 'password' && (value.length === 0 || !value)) {
          RightNow.UI.Dialog.messageDialog('Correo electrónico incorrecto', { title: 'Intente nuevamente' });
          return false;
        } else {
          this.inputField.set('value', this.inputField.get('value'));
        }
