RightNow.namespace('Custom.Widgets.Info.search_hh_status');
Custom.Widgets.Info.search_hh_status = RightNow.Widgets.extend({     /**
     * Widget constructor.
     */
    constructor: function () {
        window.search_hh_status = this;
    ...
    init: function () {
        ...
        this.status_list.input.on("keypress", this.chequer, this);
        this.status_list.input.on('change', this.chequer, this)
        this.rut_list= Integer.getInstanceByName('rut_list');

        // Carga Listas
        //this.rut_list.js.initial_value='96530470-3';
        Integer.appendOptions(this.status_list, this.data.js.list.Status, 'select', null, true);
        Integer.appendOptions(this.rut_list, this.data.js.list.Status_rut, 'select', null, true);
        this.rut_list.input.set('value',this.data.js.list.rut);
    ...
    handler_btn_get_trx: function () {
        ...
        this.rut_list = Integer.getInstanceByName('rut_list');
        data.rut_list = this.rut_list.input.get('value');
   ...
   */
    ...
    getTrx_ajax_endpointCallback: function (response, originalEventObj) {
        ...
        this.btn_get_trx.set('disabled', false);
        this.errors_container.one('.messages').setHTML('<p>' +response.Trx_data.length + ' Equipos encontrados.</p>');
    ...
    fillTables: function (data) {
        ...
        if (data) {
            ...
            for (var i = 0, count = data.length; i < count; i++) {
                ...
                //this.XX = Integer.getInstanceByName('XX');
                //this.XX.input.set('value', data.length);
                if (data[i].NUBEPRINT == "CONECTADO") {
                    btn_pay = this.Y.Node.create('<div class="circuloGreen"></div>CONECTADO');
                    container.append(btn_pay);
                    data[i].NUBEPRINT = container;
                }
                if (data[i].NUBEPRINT == "DESCONECTADO") {
                    btn_pay = this.Y.Node.create('<div class="circuloRed"></div>DESCONECTADO');
                    container.append(btn_pay);
                    data[i].NUBEPRINT = container;
                }
                if (data[i].NUBEPRINT == "NO MONITOREADO") {
                    btn_pay = this.Y.Node.create('<div class="circuloWhite"></div>NO MONITOREADO');
                    container.append(btn_pay);
