RightNow.namespace('Custom.Widgets.Info.search_supplies_status');
Custom.Widgets.Info.search_supplies_status = RightNow.Widgets.extend({     /**
     * Widget constructor.
     */
    constructor: function () {
        window.search_supplies_status = this;
    ...
    fillTables: function (data) {
        ...
        if (data) {
            ...
            for (var i = 0, count = data.length; i < count; i++) {
                ...
                var is_pending = (data[i].CONTRATO > 0) ? true : false;
                var container = this.Y.Node.create('<span></span>');
                btn_pay = this.Y.Node.create('<div><progress id="file" class="black" max="100" value="' + data[i].NIVEL_TONER_NEGRO + '"></progress>' + data[i].NIVEL_TONER_NEGRO + '%</div>');
                container.append(btn_pay);
                if (data[i].NIVEL_TONER_CIAN > 0) {
                    btn_pay = this.Y.Node.create('<div><progress id="file" class="cyan" max="100" value="' + data[i].NIVEL_TONER_CIAN + '"></progress>' + data[i].NIVEL_TONER_CIAN + '%</div>');
                    container.append(btn_pay);
                }

                if (data[i].NIVEL_TONER_MAGENTA > 0) {
                    btn_pay = this.Y.Node.create('<div><progress id="file" class="magenta" max="100" value="' + data[i].NIVEL_TONER_MAGENTA + '"></progress>' + data[i].NIVEL_TONER_MAGENTA + '%</div>');
                    container.append(btn_pay);
                }
                if (data[i].NIVEL_TONER_AMARILLO > 0) {
                    btn_pay = this.Y.Node.create('<div><progress id="file" class="yellow" max="100" value="' + data[i].NIVEL_TONER_AMARILLO + '"></progress>' + data[i].NIVEL_TONER_AMARILLO + '%</div>');
                    container.append(btn_pay);
                }

                data[i].NIVEL_TONER_NEGRO = container;

                if (!data[i].FECHA_ACTUALIZACION) {
                    btn_pay = this.Y.Node.create('<div>SIN DATOS</div>');
                    container.append(btn_pay);
                    data[i].FECHA_ACTUALIZACION = container;
                }
