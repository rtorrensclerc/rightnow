...
class search_supplies_status extends \RightNow\Libraries\Widget\Base {
  public $contactId;
  ...
  /**
    ...
    function handle_getTrx_ajax_endpoint($params) {
      ...
      $Trx=$this->CI->GeneralServices->BuscaInfoNivelesNbp($trx_rut,$trx_hh,$serie);
  
     

      if($Trx->niveles->nivel==null)
      {
          $params['Trx_data']=$Trx->niveles->nivel;
          //echo "ERROR1 ". json_encode($Trx);
      }
      else
      {
        
          if(!array_key_exists(0, $Trx->niveles->nivel))
          {
              $params['Trx_data'][0]=$Trx->niveles->nivel;
