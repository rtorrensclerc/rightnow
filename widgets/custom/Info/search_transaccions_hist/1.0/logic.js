RightNow.namespace('Custom.Widgets.Info.search_transaccions_hist');
Custom.Widgets.Info.search_transaccions_hist = RightNow.Widgets.extend({     /**
     * Widget constructor.
     */
    constructor: function() {
        window.search_transaccions_hist = this;
     ...
     */
     handler_btn_get_trx: function() {
        ...
        this.invoice_from = Integer.getInstanceByName('invoice_from');
        data.trx_from = this.invoice_from.input.get('value');

        this.invoice_to = Integer.getInstanceByName('invoice_to');
        data.trx_to = this.invoice_to.input.get('value');

        data.trx_from = data.trx_from.replace('-', '/');
        data.trx_from = data.trx_from.replace('-', '/');
        data.trx_to = data.trx_to.replace('-', '/');
        data.trx_to = data.trx_to.replace('-', '/');
        if (data.trx_from.length == 0 && data.trx_to.length == 0) {
            this.getTrx_ajax_endpoint(data);
        } else {
            if (this.validatedate(data.trx_from) &&
                this.validatedate(data.trx_to)) {
                this.getTrx_ajax_endpoint(data);
            } else {
