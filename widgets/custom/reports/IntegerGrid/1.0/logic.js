RightNow.namespace('Custom.Widgets.reports.IntegerGrid');
Custom.Widgets.reports.IntegerGrid = RightNow.Widgets.extend({
    /**
     * Widget constructor.
     */
    constructor: function() {
        if (this.data.attrs.show_paginator) {
            this._currentPage = 1;
    ...
    _onPageChange: function(i)
    {
        ...
        var pageNumber = i._currentTarget.innerHTML;
    ...
    getDefault_ajax_endpoint: function(pageNumber) {
        ...
        var eventObj = {data:{
            ...
            filters: this.data.attrs.json_filters,
    ...
    default_ajax_endpointCallback: function(response, originalEventObj) {
        ...
        // Lista
        if(this.data.attrs.display_type == 'list'){

            var list = this.Y.all(this.baseSelector + ' .resultList li');
            list.remove();

            var newList = this.Y.one(this.baseSelector + ' .resultList');

            for (i=0; i<response.result.data.length; i++ )
            {
                newList.appendChild(this.Y.Node.create('<li class="element'+i+'">'));

                var node = this.Y.one(this.baseSelector + ' .element'+i);

                for ( j=0; j<response.result.headers.length; j++ )
                {
                    if (response.result.headers[j].data_type == 99)
                        node.appendChild(this.Y.Node.create('<p><a href="'+this.data.attrs.url_per_col+response.result.data[i][j]+'">'+response.result.data[i][j]+'</a></p>'));
                    else
                        node.appendChild(this.Y.Node.create('<p>'+response.result.data[i][j]+'</p>'));
                    if (j == response.result.headers.length - 1){
                        node.append(this.Y.Node.create('</li>'));
                    }
                }
            }
        }
