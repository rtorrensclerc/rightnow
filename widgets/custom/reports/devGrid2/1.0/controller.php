...
class devGrid2 extends \RightNow\Libraries\Widget\Base {
    private $page = 0;
    private $array_operator_filter = array ('=' => 1, '!=' => 2, '<' => 3, '<=' => 4, '>' => 5, '>=' => 6, 'like' => 7, 'notlike' => 8, 'between' => 9, 'list' => 10, 'notlist' => 11, 'null' => 12, 'notnull' => 13, 'regex' => 19, 'notregex' => 20, '&' => 24);
    ...
    function getData()
    {
      ...
      if (!empty($kw)){
        $kw = $kw.'%';
        $filterArray = array();
        $filterArray[] = array("name" => 'Refno', "operator" => 'like' , "type" => 'STRING' , "value" => $kw);
        $this->buildFilters(json_encode($filterArray));
      }else if (!empty($this->data['attrs']['json_filters']))
