...
class Transactions_v2 extends \RightNow\Controllers\Base
{
    public $error;
    public $rut_org;
    public function __construct()
    {
        ...
        $this->load->model('custom/PaymentsInvoicesServices_v2');
        $this->load->model('custom/Contact');
        $this->load->helper('utils_helper');
    }

  
    public function initTransaction_back()
    {
    ...
    public function initTransaction()
    {
      ...
      $result = $this->PaymentsInvoicesServices_v2->initTransaction($invoice, $amount);
    ...
    public function validTransaction()
    {
      try
      {
        ...
        if($TBK_TOKEN)
        {
          ...
          $transaction_c->authorizacionCode = '000000';
          $transaction_c->shareNumber       = 0;
          $transaction_c->paymentTypeCode   = 0;
          $transaction_c->responseCode      = -1;
          $transaction_c->amount            = 0;
          // $transaction_c->invoice_rut       = $invoiceRut;
          $transaction_c->invoice_number    = 0;
          
          $transaction_c->cardNumber        = '0';
          $transaction_c->transactionDate   = strtotime("now");
        ...
        $finalMessage = $this->PaymentsInvoicesServices_v2->acknowledgeTransaction($token_ws);
        
        //echo '---->getTransactionResult1 [' . $finalMessage .']<-----<br>';
        if ($finalMessage !== false)
        {
         
          
          $result = $this->PaymentsInvoicesServices_v2->getTransactionResult($token_ws);
          //echo '---->getTransactionResult 2[' . $result .']<-----<br>';
          //echo '---->acknowledgeTransaction 2[' . $finalMessage.']<-----<br>';
          if ($result === false)
          {
            echo "Error ".$this->PaymentsInvoicesServices_v2->getLastError();
          }
          else
          {
        ...
        {
          //echo '---->getTransactionResult 3[' . $result .']<-----<br>';
          //echo '---->acknowledgeTransaction3[' . $finalMessage.']<-----<br>';
      ...
      catch(\Exception $e)
      {
        ...
        //echo '---->getTransactionResult4 [' . $result .']<-----<br>';
        //echo '---->acknowledgeTransaction4[' . $finalMessage.']<-----<br>';
    ...
    private function saveTransaction($json)
    {
      ...
      try
      {
        ...
        if (empty($data_json))
        {
          throw new \Exception('El contenido recibido no esta presente', 6);
        }

        //Se crea un objeto para posteriormente enviarlo al modelo
        $transaction_c                    = new \stdClass();
        $transaction_c->authorizacionCode = $data_json['authorization_code'];
        $transaction_c->shareNumber       = (int) $data_json['installments_number'];
        $transaction_c->paymentTypeCode   = $data_json['payment_type_code'];
        $transaction_c->responseCode      = $data_json['response_code'];
        $transaction_c->amount            = $data_json['amount'];
        // $transaction_c->invoice_rut       = $invoiceRut;
        $transaction_c->invoice_number    = $data_json['buy_order'];
        $transaction_c->info_json         = $json; //El json que llegó
        $transaction_c->cardNumber        = $data_json['card_detail']['card_number'];
        $transaction_c->transactionDate   = strtotime($data_json['transaction_date']);
        $transaction_c->organization_rut  = (empty(getCompanyRutBySession()))?null:getCompanyRutBySession();
        $transaction_c->contact_id        = (\RightNow\Utils\Framework::isLoggedIn())?$this->session->getProfile()->c_id->value:null;
        ...
        if($transaction_c->responseCode=='0')
        {
         ...
         /* Construir la estructura getTransactionResultResponse
 
          $getTransactionResultResponse= new \stdClass();
          $getTransactionResultResponse->return                               = new \stdClass();
          $getTransactionResultResponse->return->sessionId=
          $getTransactionResultResponse->return->accountingDate=
          $getTransactionResultResponse->return->buyOrder=
          $getTransactionResultResponse->return->detailOutput                            = new \stdClass();
          $getTransactionResultResponse->return->detailOutput->amount=
          $getTransactionResultResponse->return->detailOutput->authorizationCode=
          $getTransactionResultResponse->return->detailOutput->paymentTypeCode=
          $getTransactionResultResponse->return->cardDetail->cardNumber=
          $getTransactionResultResponse->return->transactionDate=
          $getTransactionResultResponse->return->VCI=
          $getTransactionResultResponse->return->urlRedirection=
          $getTransactionResultResponse->return->detailOutput->commerceCode=
          $getTransactionResultResponse->return->detailOutput->sharesAmount=
          $getTransactionResultResponse->return->detailOutput->sharesNumber=

          */
         
          $receipt = $this->PaymentsInvoicesServices_v2->createReceipt($json);
        }
