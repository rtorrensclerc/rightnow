 ...
 * CPM que obtiene el reporte del HH solicitado
 * y lo almacena en un campo personalizado "predictiondata" en el ticket
 */
...
class GetPredictionSuplies
{
    
    //CONST URL_GET_HH = "https://api.dimacofi.cl/DDDM/DDDM_LeeReporte";
    ...
    static function HandleIncident($runMode, $action, $incident, $cycle)
    {
        ...
        try {
            ...
            $url=$cfg2->Value .'/DDDM/DDDM_LeeReporte';
            self::insertPrivateNote($incident, $url);
            ...
            if ($result != FALSE) {
                ...
                if ($arr_json != false) {
                    ...
                    self::saveAnswer($incident, $result);
    ...
    //metodo guarda en un campo personalizado el mensaje
    static function saveAnswer($incident, $answer)
    {
        try {
            
            RNCPHP\ConnectAPI::commit();
            $incident->CustomFields->c->predictiondata = $answer; // <-- Ahi ira el campo personalizado sin nombre aun
            
            $incident->Save(); //Este save disparará la regla que guardará el mensaje
