...
class Blowfish {
  ...
  */
  function Blowfish($key) {
    $this->mode = self::BLOWFISH_MODE_EBC;
    $this->padding = self::BLOWFISH_PADDING_ZERO;
    $this->N = 16;
    $this->blockSize = 8;
    $this->_applyKey($key);
    $iv = null;
  ...
  function encryptold($plaintext, $key, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
  ...
  function decryptold($ciphertext, $key, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == Blowfish::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
